...
import {USER_SESSION_KEY, USER_BIND_SESSION_KEY} from '../shared/constants';
...
const base64Module = require('base-64/base64.js');
...
export class AuthService {
  ...
  constructor(@Inject(USER_BIND_SESSION_KEY) public sessionKey: string,
              @Inject(BackEndService) public backendService: BackEndService) {
    if (!!this.sessionKey) {
  ...
  login(username: string, password: string): Subject<boolean> {
    let encoded: string = base64Module.encode(password);
